...
class OrderController extends Controller
{
    ...
    public function index($slug, Request $request)
    {
        if ($slug == 'all') {
            ...
            $show_data = Order::with('shipping', 'status');
            if ($request->phone) {
                $show_data = $show_data->WhereHas('shipping', function ($subQuery) use ($request) {
                    $subQuery->where('phone', $request->phone);
                });
            }
            if ($request->amount) {
                $show_data = $show_data->where('amount',$request->amount);
            }
            if ($request->sort) {
                $show_data = $show_data->orderBy('amount',$request->sort);
            }else{
        ...
        if ($request->status == 6 && $order_status != 6) {
            ...
            if ($sms_gateway) {
                ...
                $data = [
                    ...
                    "message" => "Dear $request->name!\r\nYour order ($order->invoice_id) has been successfully Completed. Track your order https://priyohub.com/customer/order-track and Total Bill $order->amount\r\nThank you for using $site_setting->name"
                ];
                $ch = curl_init();
    ...
    public function bulk_destroy(Request $request)
    {
     ...
     public function slip($invoice_id){
        $order = Order::where(['invoice_id'=>$invoice_id])->with('orderdetails','payment','shipping')->firstOrFail();
        return view('backEnd.order.slip',compact('order'));
    }
    
    public function order_print(Request $request)
    {
        $orders = Order::whereIn('id', $request->input('order_ids'))->with('orderdetails', 'payment', 'shipping')->get();
