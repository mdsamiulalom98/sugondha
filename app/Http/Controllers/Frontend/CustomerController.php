   ...
   public function validateExam(Request $request)
    {
        ...
        $store_data->total_mark = $totalPossiblePoints;
    ...
    public function order_save(Request $request)
    {
        ...
        if ($sms_gateway) {
            ...
            $data = [
                ...
                "type" => 'text',
                "senderid" => "$sms_gateway->serderid",
                "message" => "Dear $request->name!\r\nYour order ($order->invoice_id) has been successfully placed. Track your order https://priyohub.com/customer/order-track and Total Bill $order->amount\r\nThank you for using $site_setting->name"
            ];
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_POST, 1);
